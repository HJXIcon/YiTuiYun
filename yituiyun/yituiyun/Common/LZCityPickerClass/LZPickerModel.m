...
- (void)configProvicWithArray:(NSArray *)array {
    ...
    NSMutableArray *tmpCitys = [NSMutableArray arrayWithCapacity:array.count];
    for (NSDictionary *tmp in array) {
        
        LZCity *city = [[LZCity alloc]init];
        city.name = tmp[@"name"];
        city.cityID = tmp[@"city_id"];
        city.province = self.name;
      
        NSArray *areas = tmp[@"child"];
        
        if (areas.count == 0) {
            
        }else{
           [city configWithArr:areas];
        }
