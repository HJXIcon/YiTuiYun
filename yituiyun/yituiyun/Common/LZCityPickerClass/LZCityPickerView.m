...
- (NSDictionary *)textAttributes {
    if (_textAttributes == nil) {
        _textAttributes = @{NSFontAttributeName:[UIFont systemFontOfSize:15],NSForegroundColorAttributeName:[UIColor blackColor]};
    }
...
- (NSDictionary *)titleAttributes {
    if (_titleAttributes == nil) {
        _titleAttributes = @{NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:[UIColor redColor]};
    }
...
- (void)loadData {
    ...
    NSArray *datas = [[NSUserDefaults standardUserDefaults] objectForKey:@"cityAddressArray"];
    
    
    
    for (NSDictionary *tmp in datas) {
        
        LZProvince *province = [[LZProvince alloc]init];
        province.name = tmp[@"name"];
        province.provinID = tmp[@"province_id"];
        
       
        
        [province configProvicWithArray:tmp[@"child"]];
...
//    if (!self.autoChange) {
        ...
        self.cancelButton.frame = CGRectMake(10, 10, 40, lz_buttonHeight );
        
//    }
    
    self.commitButton.frame = CGRectMake(lz_screenWidth - 50, 10, 40, lz_buttonHeight );
...
- (void)commitButtonClick:(UIButton *)button {
    ...
    if (self._selectBlock) {
        ...
        if ([ZQ_CommonTool isEmpty:__currentArea.name]) {
          
            __currentArea.name = @"";
             address = [NSString stringWithFormat:@"%@-%@",__currentProvience.name,__currentCity.name];
            
             __selectBlock(address,__currentProvience.provinID,__currentCity.cityID,@"");
        }else{
             address = [NSString stringWithFormat:@"%@-%@-%@",__currentProvience.name,__currentCity.name,__currentArea.name];
            
             __selectBlock(address,__currentProvience.provinID,__currentCity.cityID,__currentArea.areaID);
        }
