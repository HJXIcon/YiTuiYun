...
- (void)_setupSubview
{
    ...
    tapGR.numberOfTapsRequired = 1;       // 设置轻拍的次数
    tapGR.numberOfTouchesRequired = 1;    // 设置需要的手指数
...
- (void)setModel:(id<IUserModel>)model
{
    ...
    if ([_model.avatarURLPath length] > 0){
        [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", kHost, _model.avatarURLPath]] placeholderImage:_model.avatarImage];
    } else {
...
- (void)headerLongPress:(UILongPressGestureRecognizer *)longPress
{
    if (longPress.state == UIGestureRecognizerStateBegan) {
        if(_delegate && _indexPath && [_delegate respondsToSelector:@selector(cellImageViewLongPressAtIndexPath:WithTableViewTag:)])
        {
            [_delegate cellImageViewLongPressAtIndexPath:self.indexPath  WithTableViewTag:_tableViewTag];
        }
    }
}
...
{
    if(_delegate && _indexPath && [_delegate respondsToSelector:@selector(portraitButtonClickAtIndexPath:WithTableViewTag:)])
    {
        [_delegate portraitButtonClickAtIndexPath:self.indexPath  WithTableViewTag:_tableViewTag];
    }
}
