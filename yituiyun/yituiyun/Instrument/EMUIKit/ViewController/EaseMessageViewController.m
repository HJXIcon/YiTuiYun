...
@property (nonatomic, strong) NSString *groupManager;
...
- (void)getGroupPersonnel{
    [[EMClient sharedClient].groupManager getGroupSpecificationFromServerByID:self.conversation.conversationId includeMembersList:NO completion:^(EMGroup *aGroup, EMError *aError) {
        if (!aError) {
            if (aGroup) {
                            ...
                            username:(NSString *)aUsername
{
    ...
    [self showHint:[NSString stringWithFormat:@"\'%@\'进入\'%@\'", aUsername, aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}
...
{
    CGRect frame = self.chatToolbar.frame;
    [self showHint:[NSString stringWithFormat:@"\'%@\'离开 chatroom\'%@\'", aUsername, aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}
                              ...
                              reason:(EMChatroomBeKickedReason)aReason
{
    if ([_conversation.conversationId isEqualToString:aChatroom.chatroomId])
    {
        _isKicked = YES;
        CGRect frame = self.chatToolbar.frame;
        [self showHint:[NSString stringWithFormat:@"您已退出\'%@\'", aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
                                  ...
                                  read:(BOOL)read
{
    ...
    if (((body.type == EMMessageBodyTypeVideo) ||
         ...
         (body.type == EMMessageBodyTypeFile)) &&
        !read)
    {
        return NO;
    }
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    [self showHudInView:self.view hint:@"加载视频..."];
...
- (void)_fileMessageCellSelected:(id<IMessageModel>)model
{
    ...
    EMFileMessageBody *videoBody = (EMFileMessageBody*)model.message.body;
    
    NSString *localPath = [model.fileLocalPath length] > 0 ? model.fileLocalPath : videoBody.localPath;
    if ([localPath length] == 0) {
        [self showHint:@"下载文件失败!"];
        return;
    }
    ...
    dispatch_block_t block = ^{
        ...
        [self showHint:@"下载文件成功"];
    };
    
    __weak typeof(self) weakSelf = self;
    [self showHudInView:self.view hint:@"下载文件..."];
    [[EMClient sharedClient].chatManager downloadMessageAttachment:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
        [weakSelf hideHud];
        if (!error) {
            block();
        }else{
            [weakSelf showHint:@"下载文件失败!"];
        }
    }];
}
...
//        if ([model.message.from isEqualToString:self.groupManager]) {
//            if (([model.nickname rangeOfString:@"项目主管-"].location == NSNotFound) || ([model.nickname rangeOfString:@"发起人-"].location == NSNotFound)) {
//                UserInfoModel *userInfoModel = [ZQ_AppCache userInfoVo];
//                if ([userInfoModel.identity isEqualToString:@"6"]) {
//                    model.nickname = [NSString stringWithFormat:@"项目主管-%@", model.nickname];
//                } else if ([userInfoModel.identity isEqualToString:@"5"]) {
//                    model.nickname = [NSString stringWithFormat:@"发起人-%@", model.nickname];
//                }
//            }
//        }
        sendCell.model = model;
...
//            }
        ...
        if (message.chatType == EMChatTypeChat) {
            ...
            NSDictionary *dic = [ZQ_AppCache takeOutFriendInfo:message.from];
            if (dic) {
                ...
                if ([model.message.from isEqualToString:self.groupManager]) {
                    ...
                    if ([userInfoModel.identity isEqualToString:@"6"]) {
                        if (self.groupType == EMGroupStylePublicJoinNeedApproval) {
                            model.nickname = [NSString stringWithFormat:@"群主-%@", dic[@"nickname"]];
                        } else {
                            model.nickname = [NSString stringWithFormat:@"项目主管-%@", dic[@"nickname"]];
                        }
                    } else if ([userInfoModel.identity isEqualToString:@"5"]) {
                        if (self.groupType == EMGroupStylePublicJoinNeedApproval) {
                            model.nickname = [NSString stringWithFormat:@"群主-%@", dic[@"nickname"]];
                        } else {
                            model.nickname = [NSString stringWithFormat:@"发起人-%@",dic[@"nickname"]];
                        }
                    }
                } else {
                     ...
                     progress:(id)progress
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        EaseMessageModel *model = messages.lastObject;
        dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([message.messageId isEqualToString:model.messageId]) {
                    ...
                    if (message.chatType == EMChatTypeChat) {
                        ...
                        if (dic) {
                            ...
                            [self userAvatar:model WithIndex:i];
                        }
                    }
                    break;
                }
...
- (void)userAvatar:(EaseMessageModel *)model WithIndex:(NSUInteger)index
{
    EMMessage *aMessage = model.message;
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"uids"] = aMessage.from;
    NSString *URL = [NSString stringWithFormat:@"%@%@", kHost, @"api.php?m=user.avatars"];
    [HFRequest requestWithUrl:URL parameters:params success:^(NSURLSessionDataTask * _Nullable task, id  _Nullable responseObject) {
        if ([responseObject[@"errno"] intValue] == 0) {
            NSDictionary *infoDic = responseObject[@"rst"][aMessage.from];
            ...
            [ZQ_AppCache saveUserFriendInfo:infoDic WithName:aMessage.from];
...
- (void)userAvatar:(EaseMessageModel *)model
{
    EMMessage *aMessage = model.message;
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"uids"] = aMessage.from;
...
- (void)_updateMessageStatus:(EMMessage *)aMessage
{
    ...
    if (aMessage && isChatting) {
        ...
        if (aMessage.chatType == EMChatTypeChat) {
            ...
            NSDictionary *dic = [ZQ_AppCache takeOutFriendInfo:aMessage.from];
            if (dic) {
                model = [[EaseMessageModel alloc] initWithMessage:aMessage];
                model.avatarURLPath = [NSString stringWithFormat:@"%@%@", kHost, dic[@"avatar"]];
                model.avatarImage = [UIImage imageNamed:@"EaseUIResource.bundle/user"];
