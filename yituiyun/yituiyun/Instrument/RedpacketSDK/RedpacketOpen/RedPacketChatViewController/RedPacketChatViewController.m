...
- (void)viewDidLoad
{
    ...
    self.viewControl = [[RedpacketViewControl alloc] init];
    /** 获取群组用户代理 */
...
- (RedpacketUserInfo *)profileEntityWith:(NSString *)userId
{
    ...
    NSDictionary *dic = [ZQ_AppCache takeOutFriendInfo:userId];
    if (dic) {
        NSString *string = dic[@"nickname"];
        if (![ZQ_CommonTool isEmpty:string]) {
            userInfo.userNickname = string;
        } else {
    ...
    userInfo.userAvatar = [NSString stringWithFormat:@"%@%@", kHost, dic[@"avatar"]];
