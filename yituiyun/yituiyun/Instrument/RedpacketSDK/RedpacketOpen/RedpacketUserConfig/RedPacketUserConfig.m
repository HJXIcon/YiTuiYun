...
- (RedpacketUserInfo *)redpacketUserInfo
{
    ...
    UserInfoModel *entity = [ZQ_AppCache userInfoVo];
    NSString *nickname = entity.nickname;
    userInfo.userNickname = nickname.length > 0 ? nickname : userInfo.userId;
    userInfo.userAvatar = [NSString stringWithFormat:@"%@%@", kHost, entity.avatar];
    return userInfo;
}
    ...
    for (EMMessage *message in aCmdMessages) {
        ...
        if ([body.action isEqualToString:RedpacketKeyRedapcketCmd]) {
            ...
            if ([senderID isEqualToString:currentUserID]){
                /** 当前用户是红包发送者 */
                NSString *text = [NSString stringWithFormat:@"%@领取了你的红包",[dict valueForKey:RedpacketKeyRedpacketReceiverNickname]];
