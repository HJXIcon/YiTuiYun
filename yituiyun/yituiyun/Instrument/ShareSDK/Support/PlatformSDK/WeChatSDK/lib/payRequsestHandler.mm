...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demoWithDict:(NSMutableDictionary *)dictionary
{

    //订单标题，展示给用户
    NSString *order_name    = [NSString stringWithFormat:@"%@", dictionary[@"orderBody"]];
    //订单金额,单位（分）
    NSString *order_price   = [NSString stringWithFormat:@"%@", dictionary[@"payAmount"]];
    //支付结果回调页面
    NSString *notify_url = [NSString stringWithFormat:@"%@%@", kHost, dictionary[@"notifyUrl"]];
    
    NSArray *array = [getIPhoneIP getIpAddresses];
    NSString *string;
    if ([ZQ_CommonTool isEmptyArray:array]) {
        string = [getIPhoneIP getIPAddress];
    } else {
    ...
    NSString *orderno   = [NSString stringWithFormat:@"%@", dictionary[@"orderCode"]];;
    ...
    [packageParams setObject: string            forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: order_price       forKey:@"total_fee"];       //订单金额，单位为分
//
//    //获取prepayId（预支付交易会话标识）
    NSString *prePayid = [NSString stringWithFormat:@"%@", dictionary[@"prepayId"]];
