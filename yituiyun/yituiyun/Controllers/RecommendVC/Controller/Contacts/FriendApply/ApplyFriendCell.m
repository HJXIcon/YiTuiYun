...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [_addButton setBackgroundColor:MainColor];
        ...
        [_refuseButton setBackgroundColor:[UIColor whiteColor]];
        _refuseButton.layer.borderWidth = 1;
        _refuseButton.layer.borderColor = MainColor.CGColor;
        [_refuseButton setTitle:@"拒绝" forState:UIControlStateNormal];
        _refuseButton.clipsToBounds = YES;
        [_refuseButton setTitleColor:MainColor forState:UIControlStateNormal];
...
+ (CGFloat)heightWithContent:(NSString *)content
{
    if (!content || content.length == 0) {
        ...
        CGSize size = [content boundingRectWithSize:CGSizeMake(ZQ_Device_Width - 60 - 120, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size;
