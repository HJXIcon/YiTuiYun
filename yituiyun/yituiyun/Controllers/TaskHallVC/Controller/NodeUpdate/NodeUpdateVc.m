...
@interface NodeUpdateVc ()<UITableViewDataSource,UITableViewDelegate,ChooseImageCellDelegate,InputTextFieldViewControllerDelegate,ShowImageViewControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,ZYQAssetPickerControllerDelegate>
...
-(TaskNodeHeadView *)headView{
    if (_headView == nil) {
        ...
        _headView.SecondBtn.selected = YES;
        _headView.arrorw2Btn.selected = YES;
...
//        _titleInfoArray =[NSMutableArray arrayWithObjects:@"serc+",@"serc+",@"serc+",@"serc+",@"serc+",@"serc+",@"serc+", nil];
//        
//    }
...
//    [self.view endEditing:NO];
    ...
    //        InputTextFieldViewController *vc = [[InputTextFieldViewController alloc] initWithTitle:model.taskName];
    //        vc.delegate = self;
    //        vc.index = indexPath;
...
- (void)uploadButtonClick
{
    ...
    __weak NodeUpdateVc *weakSelf = self;
    ...
    if ([ZQ_CommonTool isEmptyDictionary:params[@"imgs"][@"place_img_list"]]) {
        
        [weakSelf showHint:@"请选择上传的照片"];
        return ;
    }
