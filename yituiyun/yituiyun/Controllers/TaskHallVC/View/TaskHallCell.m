...
- (void)setModel:(ProjectModel *)model
{
    ...
    if ([model.status integerValue] == 1 || [model.status integerValue] == 2 ||  [model.status integerValue] == 6 || [model.status integerValue] == 7 || [model.status integerValue] == 8) {
        _button.hidden = YES;
        ...
        if ([model.status integerValue] == 0 || [model.status integerValue] == 5) {
            [_button setTitle:@"开始任务" forState:UIControlStateNormal];
            _button.tag = 1041;
        }
        if ([model.status integerValue] == 3) {
            [_button setTitle:@"重新开始任务" forState:UIControlStateNormal];
...
- (void)buttonClick:(UIButton *)button{
    ...
    } else if (button.tag == 1042) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(cancelButtonClickWithIndex:)]) {
            [self.delegate cancelButtonClickWithIndex:_indexPath];
        }
    ...
    }else if (button.tag = 1045){
        if ([self.delegate respondsToSelector:@selector(restartTaskWith:wihtT_demanType:)]) {
            [self.delegate restartTaskWith:_model.projectId wihtT_demanType:_model.t_demandTyp];
        }
