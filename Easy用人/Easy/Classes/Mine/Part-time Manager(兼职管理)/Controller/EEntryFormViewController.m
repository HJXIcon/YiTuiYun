...
@interface EEntryFormViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@implementation EEntryFormViewController
...
- (EStatisticalTableHeaderView *)headerView{
    if (_headerView == nil) {
        ...
        JXWeak(self);
        _headerView.hiddenPeopleNumLabel = NO;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    /// 获取已在项目中的团队成员userId
    [EEntryFormViewModel demandGroupMemberWithDemandPriceId:self.demandPriceId completion:^(NSArray<NSString *> *userIds) {
        
        self.headerView.peopleNum = userIds.count;
        self.existUserIds = userIds;
    }];
}
...
- (void)setupUI{
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"邀请" style:UIBarButtonItemStyleDone target:self action:@selector(rightAction)];
...
- (void)setupRefresh{
    ...
    MJRefreshNormalHeader *header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [weakSelf.viewModel demandGroupManageWithDemandPriceId:self.demandPriceId targetUserId:nil startTime:weakSelf.startTime endTime:weakSelf.endTime keys:weakSelf.keys completion:^{
...
- (void)rightAction{
    EMyTeamViewController *vc = [[EMyTeamViewController alloc]init];
    vc.isDemandAddMember = YES;
    vc.demandPriceId = self.demandPriceId;
    vc.existUserIds = self.existUserIds;
