//
//  EThirdLoginBindViewController.m
//  Easy
//
...
@interface EThirdLoginBindViewController ()<UITextFieldDelegate>
...
@implementation EThirdLoginBindViewController
...
- (void)setupUI{
    ...
    [self.boxBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(line1.mas_bottom).offset(E_RealHeight(20));
    }];
    ...
    // 绑定
    self.bindBtn = [JXFactoryTool creatButton:CGRectZero font:[UIFont systemFontOfSize:18] normalColor:[UIColor whiteColor] selectColor:nil title:@"绑定" nornamlImageName:nil selectImageName:nil textAlignment:NSTextAlignmentCenter target:self action:@selector(bindAction)];
    ...
    [self.bindBtn setBackgroundImage:nornamlImage forState:UIControlStateNormal];
    [self.bindBtn setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithHexString:@"#ffba00"]] forState:UIControlStateHighlighted];
    self.bindBtn.cornerRadius = E_RealHeight(25);
    [self.bindBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(line1.mas_bottom).offset(E_RealHeight(64));
...
- (void)bindAction{
    ...
    if (!self.boxBtn.selected) {
        [self showHint:@"需要同意协议才能绑定"];
        return;
    }
    
    [ELoginViewModel wxBindMobile:self.phoneTextF.text verifyCode:self.codeTextF.text wxUid:self.wxUid completion:^(BOOL isSuccess) {
        if (isSuccess) {
